...
def prepare_data(data):
    ...
    train_data, test_data = adj_close_normalized[:train_size], adj_close_normalized[train_size:]
...
def visualize_results(data, scaler, model, stock_symbol, look_back=1, filter_date=None):
    # Making predictions for train data
    data = data.reshape(-1, 1, 1)
    train_predict = model.predict(data)
    train_predict = scaler.inverse_transform(train_predict)

    # Converting the index to datetime format and filtering by date
    dates = pd.date_range(start=START_DATE, end=END_DATE, periods=len(data))
    if filter_date:
        mask = dates >= filter_date
        dates = dates[mask]
    ...
    plt.plot(dates, scaler.inverse_transform(data.reshape(-1, 1)), label=f'Actual ({stock_symbol})', color='blue', alpha=0.6, linewidth=2)
...
def main():
    for symbol in SYMBOLS:
        ...
        visualize_results(train_data, scaler, lstm_model, symbol, LOOK_BACK, FILTER_DATE)

if __name__ == "__main__":
    main()
```

![Predicted vs Actual Stock Prices-100epoch+FILTER-06 01 24](https://github.com/MLiserb/Public_articles/assets/144083324/39887002-e727-44f0-8f01-976e40329c64)
